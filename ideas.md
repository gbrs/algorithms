...
Кратко:  
Какого минимального размера квадратную доску надо взять, чтобы разместить на ней N дипломов размера H*W?  
Ответ на задачу должен меняться монотонно. r - заведомо подходит, l - нет. Левый бинарный поиск по ответу. 
...
Кратко:  
Решить уравнение с какой-то точностью.  
Функция д.б. монотонная. 
Вещественный бинарный поиск по ответу. 
Использовать for. int - 2^32 уровней. С запасом 50 итераций.
...
Функция имеет только один минимум. Ветви монотонные.
Тернарный (троичный) поиск. 
Точки ml правее l на 1/3, mr - на 2/3. 
Если ml > mr, l = ml. 
Итераций больше, чем для бинарного.
...
Кратко:
Найти время, когда все кассы работают.  
Кортежи (-1, координата начала) или (1, координата конца) сортируем.   
Считаем cnt -= tpl[0].  
Числа в кортежах в зависимости от особенностей счёта.  
---
...
Кратко:
Подсчет уникальных элементов в GA4.  
HyperLogLog.  
Вычисляем хеши значений, считаем максимальное встретившееся кол-во нулей подряд, делаем оценку кол-ва уников.  
Предварительно делим на бакеты (можно на разных машинах), в конце среднее гармоническое. 
...
Например, Алгоритм Кнута-Морриса-Пратта. O(n+m).  
Наивный подход - сравниваем элементы, начиная с s[0], потом с s[1] -> O(n * m).  
Почему смещаемся на 1 элемент, а не на m? 
Потому что внутри s' могут быть элементы, которые похожи на начало s'.  
Алгоритм эти повторы предподсчитывает и потом использует для определения смещения.  
