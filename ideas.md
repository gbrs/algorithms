...
Создаем массив лидеров: кто является лидером данного элемента - заполняем его от 1 до n - 1 поскольку сейчас 
каждый элемент сам себе лидер.  
...
Теперь оптимизации.  
Эвристики сжатия пути. При каждом запросе второго типа все вершины попавшиеся на пути запоминаем и потом подсоединяем 
напрямую к лидеру. Просто пишется рекурсивно, но можно и список пройденных вершин копить и потом вершины из него 
переподсоединить к лидеру. 
...
Надо хранить массив рангов лидеров.  
Весовая эвристика. Меньшее дерево подвешиваем к большему. Надо хранить массив весов для лидеров. 
