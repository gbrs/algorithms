...
Сделать краткое описание алгоритмов для деревьев  
...
Встречай число k прибавляем 1 к этому счетчику. 
...
Кратко:  
Обход дерева. На достижимость часто.  
DFS. Бежим до левого нижнего листа, потом шаг назад и шаг правее.  
Используем список смежности, ведем список уже посещенных вершин.  
Для шагов каждый раз вызываем рекурсивно алгоритм. Можно и стек явный использовать.  
...
Кратко:  
Cколько вершин можно достичь из заданной вершины?  
dfs. В массиве посещенных вершин будут отмечены только достижимые вершины.
...
Кратко:  
Поиск/раскраска/подсчет компонент связности  
Бежим циклом по вершинам в списке посещенных вершин и запускаем dfs от каждой еще не посещенной вершинки. 
В спв отмечаем к какой из компонент связности относится данная вершина.   
...
dfs. Сначала все вершины белые. При первом посещении (в начале функции дфс) красим в серый, 
при последнем (в конце функции дфс) - в черный.  
Цикл - это когда мы встречаем серую вершину.  

Кратко:  
Задачи, требующие предъявить подходящий путь в графе.  
Создаем массив для хранения на i-й позиции родителя i-й вершины. 
Остается пробежаться по массиву родителей от выхода до входа.  
...
Кратко:  
Поиск циклов в неориентированном графе  
dfs. Цикл - это когда мы встречаем посещенную вершину. Но надо передавать в функцию дфс номер родителя, 
чтобы не было циклов типа 1 - 2 - 1.  
...
Кратко:  
Двудолен ли граф?
Решается "жадно" dfsом. 
Идем дфсом и красим вершины попеременно в 1 и 2 (в массиве посещенных вершин). 
Если встречаем посещенную вершину, то проверяем, окрашена ли она в противоположный цвет = 3 - цвет текущей вершины. 
...
Кратко:  
Нахождение кратчайшего пути в ориентированном ацикличном графе.  
Топологическая сортировка. dfs. Складываем в список вершины при последнем посещении узла (в конце ф-ции дфс). 
Переворачиваем список в конце. Все ребра в нем направлены слева-направо.  
Создаем "обратный" список смежности (осс): предки вершин.
Внешний цикл - по вершинам топсорта, внутренний - по родителям текущей вершины. 
Пробуем прорелаксировать ребро dist[i] = min(dist[i], dist[j] + длина i-j), как в Форде-Беллмане.  

Кратко:  
Нахождение компонент сильной связности (из каждого узла в каждый) в ориентированном ацикличном графе.  
Создаем граф, в котором все ребра направлены в противоположном направлении. 
Топологическая сортировка этого графа. dfs. Складываем в список вершины при последнем посещении узла (в конце ф-ции дфс). 
Переворачиваем список в конце. Все ребра в нем направлены слева-направо.  
Запускаем dfs на исходном графе, но в порядке полученного топсорта. 
Отмечаем достижимые компоненты номером 1. 
Теперь строим ксс для следующей в топсорте вершины, которую мы еще не посетили -  номер 2.  
...
Кратко:  
Задачи с двумерной картой, где 0 - вода, 1 - суша. Или комната со стенками (лабиринт).  
Массив посещенных вершин будет матрицей такого же размера (можно использовать исходную карту). 
Надо будет обходить 4/8 соседей клетки и лучше заранее создать список кортежей[(0, 1), (0, -1), (-1, 0), (1, 0)].
Внутри функции дфс отмечаем клетку посещенной и бежим по созданным кортежам и прибавляем значения из них 
к координатам текущей клетки. 
Если получившаяся соседняя клетка не выходит за границы, не посещена и является сушей, то запускаем от нее рекурсивно 
дфс.
...
Кратко:  
Задача с двумерной картой, где 0 - вода, 1 - суша. Острова "раскрасить"  
Снаружи цикл по списку посещенных клеток. Если клетка не посещалась и является сушей, то запускаем от нее дфс 
и "красим" соседей в один цвет.
...
Кратко:  
Обход невзвешенного графа. На кратчайший путь часто.  
BFS. Обходим ближайших соседей каждой вершины.  
Используем список смежности.  
Массив расстояний инициализирован бесконечностями, старт - 0.  
Из начала очереди достаем вершину, ее соседей (еще непосещенных - у них расстояние не бесконечность) складываем в конец. 
Добавляем им в массив дистанций: расстояние до родительницы + 1.  

Кратко:  
Невзвешенный граф. Ближайшее расстояние от стартовой до других вершин.  
BFS. Массив расстояний инициализирован бесконечностями, старт - 0.  
При добавлении вершины в конец очереди добавляем для нее в массив дистанций: расстояние до родительницы + 1.
...
Кратко:
Найти все вершины/ребра, лежащие на всех кратчайших путях из A в B.  
Запускаем бфс из A в B. Пусть расстояние 4. Запускаем бфс из B в A. 
На кратчайших путях лежат все вершины, для которых сумма расстояний в этих массивах равна 4. 
Ребро лежит на кратчайшем пути, если сумма расстояний от одной вершины до A и от другой до B равна 3.  
...
Кратко:  
Задача с комнатой со стенками. Шаг на пустую клетку бесплатен, на стражника - штраф. Найти минимальную плату от входа 
до выхода.  
0-1-BFS. Массив расстояний будет матрицей такого же размера (можно использовать исходную карту). 
Надо будет обходить 4/8 соседей клетки (i, j) и лучше заранее создать список кортежей[(0, 1), (0, -1), (-1, 0), (1, 0)].
При бфс бежим по созданным кортежам и прибавляем значения из них к координатам текущей клетки. 
Если получившаяся соседняя клетка не выходит за границы, не посещена и не является стенкой, то добавляем в дэк.  
В бфс сначала должны быть обработаны более близкие узлы и только после - дальние. 
Поэтому вместо очереди - дэк. Ячейки без стражника пушим не справа, слева.  
...
Кратко:
Граф без отрицательных ребер. Найти минимальные расстояния до всех вершин из исходной 
Поддерживаем массив (в C++ set) пар "вершина-расстояние", массив уже обработанных вершин (не тратим время на них), 
массив расстояний. 
Инициализируем массив расстояний бесконечностями, у стартовой вершины - 0. 
Берем вершину с наименьшим расстоянием (на первом шаге это стартовая вершина), добавляем в массив обработанных, 
пытаемся прорелаксировать расстояние до всех еще необработанных ее соседей.
...
Кратко:
Найти минимальные расстояния до всех вершин от стартовой в графе с отрицательными ребрами (без отрицательных 
циклов)  
Инициализируем массив расстояний бесконечностями, у стартовой вершины - 0. 
Пробегаемся по всем ребрам. Если расстояние до начальной вершины бесконечность, ничего не делаем. Если нет, 
то пытаемся прорелаксировать расстояние до конечной вершины. 
Делаем V - 1 раз на случай "бамбука". 
Оптимизации. Завести флаг и брекнуться, если на предыдущей итерации не было изменений. Или SPFA: 
на первом шаге обработать только соседей стартовой вершины, а на последующих шагах релаксировать только соседей вершин, 
которые были обновлены на предыдущем шаге (очередь нам в помощь). 
...
Кратко:
Построить матрицу минимальных расстояний между всеми вершинами в графе с отрицательными ребрами (без отрицательных 
циклов)  
Строим матрицу смежности d-1[i, j]. На главной диагонали - нули. Если пути нет, записываем бесконечность.  
Сравниваем уже имеющиеся пути с путем из i в k, а потом из k в j, релаксируем расстояния, получаем матрицу d0[i, j].  
Итого 3 вложенных цикла (снаружи - по k: динамическое программирование) с условием внутри. 
...
Вводим координаты в список с кортежиками: (-1, координата начала) или (1, координата конца). 
