...
x - это величины, которые мы храним, а y - это рандомная величина, благодаря которой мы получим O(log n)
...
Выбираем в качестве корня то дерево, у которого Y в корне больше, и рекурсивно вызываем себя от другого дерева 
и соответствующего сына выбранного дерева. (?)  
...
Наконец, операция Union (T1, T2). Теоретически её асимптотика O (M log (N/M)), однако на практике она работает 
