...
lazy propagation - ленивое распространение. Очень важно научиться её писать — она часто встречается на олимпиадах.  
...
Непосредственно спускаться до листьев мы не будем.  
Когда нам позже понадобятся правильные значения таких вершин и их детей, 
мы будем делать «проталкивание» информации из текущей вершины в её сыновей: 
если метка стоит, пересчитаем сумму текущего отрезка и передадим эту метку сыновьям. 
...
В случае суммы нам даже и не надо проталкивать, а просто хранить в узлах значения 
и при get(k) просто суммировать все вершинки по пути к вершине k? 
А при замене на k всех чисел на отрезке, которые меньше k, инициализируем все вершинки дерева -inf 
и потом записываем в них максимальные значения между k и тем, что уже хранится в вершинке. 
...
Вызывать её стоит в самом начале обработки любого запроса — тогда она гарантирует, 
что в текущей вершине и её сыновьях все значения корректны.  
Какие функции кроме sum можно так обрабатывать? Коммутативность ( f(x, y) = f(y, x) )
