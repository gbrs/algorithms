...
Гамильтонов путь, гамильтонов цикл и задача о коммивояжере - Это NP-полные задачи.  
...
Только общее направление идеи для задачи коммивояжера. 
dp[i][mask] - уже найденный оптимальный путь от i-ой вершины до 0-ой, 
проходящий через вершины, отмеченные бинарной маской.  
Начальное состояние — находимся в 0-й вершине, пройденный путь равен 0, i = 0, mask = 0.  
Переход. Выбираем минимальный результат или бесконечность, если решения для данной подзадачи нет. 
Используются побитовые операции с масками.
