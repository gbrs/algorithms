...
Перенести в файлики:
С битсетом можно проводить побитовые операции, что значительно ускоряет исполнение классического рюкзака, например.
dp = dp | (dp >> a[i])
...
Делим рюкзак на 2 половины. На одну накладываем все возможные маски - берем/не берем в набор и запихиваем в 
set, unordered_set или массив (который потом отсортируем и бин поиском работать будем?). 
