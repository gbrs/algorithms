...
dp[i][0] - максимальный вес паросочетание в поддереве i-й вершины, если мы еще не брали ребро в сыновей i-й вершины; 
dp[i][1] - , соответственно, если взяли. 
Тогда, если не брали ребро в сыновей, то можем взять любой вес из сыновей 
(в них как могло приходить ребро, так и нет):  
dp[i][0] = ∑ max(dp[v][0], dp[v][1]), где v - сыновья вершины i, которых мы перебираем.  
Если же брали ребро в сына, то из него можем взять только вес dp[v][0], а если не брали, то любой из весов.
dp[i][1] = max{j∈sons(i)} ( ( ∑{v∈sons(i),v!=jmax}(dp[v][0],dp[v][1]) ) + w[i][j] + dp[j][0] ).  
